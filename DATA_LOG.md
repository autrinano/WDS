...
- **2026-07-23** — Supporting EDA update — Added a focused within-state correlation heatmap, a six-factor California–Florida boxplot figure, and three goal-aligned relationship scatterplots. The key-chart guide now contains six core narrative charts and five supporting charts.
- **2026-07-23** — Study-wide EDA refocus — Added an all-factor association screen covering all 51 eligible numeric factors and a category-level association summary. Reorganized the 11-chart presentation sequence so the overall outcome, full factor set, category patterns, collinearity, missingness, and data coverage come before four mechanism-specific follow-ups. Individual scatterplots remain a diagnostic appendix. These outputs describe associations and do not estimate causal impacts or independent contributions.
- **2026-07-24** — CoC/LASSO preparation — Added official HUD CoC PIT outcomes for 2010–2025 and a reproducible FY2024 tract-population crosswalk that allocates county predictors to CoCs. Built 1,115 CoC-year outcome rows, 974 prior-year/next-year candidate rows, and a 904-row complete core panel. The primary target is next-year PIT homelessness per 10,000 estimated CoC residents; 2021 is excluded as a target. All validation checks pass. See `coc_analysis/README.md`.
